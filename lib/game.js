    ...
    constructor(score, depth) {
        ...
        loop.update = function () {
            ...
            if (level.map[keyForPlayer] === tiles.map.stairs) {
                score.addPoints(100);
                // start a new game
                self = new Game(score, self.depth + 1);
            }
        ...
        loop.draw = function () {
            if (self.end) {
                render.drawEnd(depth, score);
                io.triggerEnd = function() {
                    location.reload();
                };
            } else {
                render.draw(player, level, camera, self, score);
            }
        };
        ...
        for (let i = 0, len = level.mobs.length; i < len; i++) {
            ...
            if (coords[0] === mob.position.x && coords[1] === mob.position.y) {
                ...
                if (counter) {
                    ...
                    let killScore = level.removeDeadMobs();
                    score.addKill(killScore);
                }
...
export default new Game(score, 1);
